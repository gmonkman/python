...
class InitData(object):
    ...
    def __init__(self):
        ...
        sql = "" \
            ...
            " inner join sample_header on sampleheader.sampleheaderid=sample.sample_headerid" \
            ...
            " where" \
            " sample.species='bass' and sample_headerid <= 3"
            #<= 3 for sample header is just bass
    ...
    def perspective_adjust(self):
        ...
        #TODO Add col, being the final corrected depth accounting for the profile of the fish


    def add_lens_sub_estimates(self):
        '''() -> void
        ...
        '''
        assert isinstance(self.df_lengths, pd.DataFrame)
        def from_cam(f, px_x, px_y, x_mm, y_mm):
            return perspective.subjdist_camera(perspective.Camera(f, px_x, px_y, x_mm, y_mm), perspective.Measure(marker_length_mm, marker_length_px))
        #need to write marker length in px to the database
