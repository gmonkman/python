...
class InitData(object):
    ...
    def perspective_adjust(self):
        ...
        colinds = pdl.cols_get_indexes_from_names(
            self.df_lengths,
            'lens_subject_distance',
            'fish_depth_from_actual',
            'lens_correction_mm')
        pdl.col_calculate_new(
            self.df_lengths,
            cm.get_perspective_correction,
            'tl_corrected_mm',
            *colinds)
        ...
        colinds = pdl.cols_get_indexes_from_names(
            self.df_lengths,
            'lens_subject_distance',
            'fish_depth_from_estimate',
            'lens_correction_mm')
        pdl.col_calculate_new(
            self.df_lengths,
            cm.get_perspective_correction,
            'estimate_corrected_mm',
            *colinds)

        # create new col in dataframe and fill with perspective corrections
        # based on the estimated fish length
        ...
        pdl.col_calculate_new(
            self.df_lengths,
            cm.get_perspective_correction_iter_linear,
            'perspective_corrected_estimate_iter_mm',
            *colinds)
    ...
    def add_depths(self):
        ...
        pdl.col_calculate_new(
            self.df_lengths,
            InitData._get_fish_depth,
            'fish_depth_from_actual',
            colinds)

        # add fish depth based on the estimated length
        colinds = pdl.cols_get_indexes_from_names(
            self.df_lengths, 'estimate_mm')
        pdl.col_calculate_new(
            self.df_lengths,
            InitData._get_fish_depth,
            'fish_depth_from_estimate',
    ...
    def update_length(self):
        ...
        # first do
        tl_cor_ind = pdl.cols_get_indexes_from_names(
            self.df_lengths, 'tl_corrected_mm')
        for i, row in self.df_lengths.iterrows():
            tl_cor = row[tl_cor_ind]
            sam_len = _SESSION.query(SampleLength).filter_by(
                sample_lengthid=int(i)).first()
            assert isinstance(sam_len, SampleLength)
            sam_len.perspective_corrected_actual_mm = _read_range_int(tl_cor)

        est_cor_ind = pdl.cols_get_indexes_from_names(
            self.df_lengths, 'estimate_corrected_mm')
        for i, row in self.df_lengths.iterrows():
        ...
        est_cor_ind = pdl.cols_get_indexes_from_names(
            self.df_lengths, 'perspective_corrected_estimate_iter_mm')
        for i, row in self.df_lengths.iterrows():
            tl_cor = row[est_cor_ind]
            sam_len = _SESSION.query(SampleLength).filter_by(
                sample_lengthid=int(i)).first()
            assert isinstance(sam_len, SampleLength)
            sam_len.perspective_corrected_estimate_iter_mm = _read_range_int(
                tl_cor)
