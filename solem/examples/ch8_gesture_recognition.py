...
from solem.PCV.classifiers import knn, bayes
...
def read_gesture_features_labels(path):
    ...
    featlist = [os.path.join(path, f)
                for f in os.listdir(path) if f.endswith('.dsift')]
...
if False:
    ...
    from solem.PCV.tools import pca
...
if True:
    ...
    res = array([knn_classifier.classify(test_features[i], k)
                 for i in range(len(test_labels))])  # TODO kan goras battre
