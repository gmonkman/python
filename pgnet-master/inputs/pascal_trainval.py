...
Extracts the annotated bouding boxes from the PASCAL dataset (out_pth).
...
def main(argv):
    ...
    voc_pth = os.path.normpath(argv[0])
    out_pth = os.path.normpath(argv[1])
    if not os.path.exists(voc_pth):
        print("{} does not exists".format(voc_pth))
        return 1
    ...
    out_path = os.path.normpath(os.path.abspath(out_pth))
    current_dir = os.path.normpath(os.path.abspath(os.getcwd()))

    ts_csv_abs = os.path.normpath("{}/ts.csv".format(out_path))
    ...
    with open(ts_csv_abs, mode='w') as csv_file:
        ...
        images_path = "{}/JPEGImages/".format(voc_pth)
        ...
        for current_class in CLASSES:
            ...
            for line in lines:
                ...
                if splitted[1] == "-1":
                    continue
                image_xml = "{}/Annotations/{}.xml".format(voc_pth, splitted[0])
