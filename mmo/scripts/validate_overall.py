...
CSV = 'C:/Users/Graham Monkman/OneDrive/Documents/MMOMapping/survey/_results'
...
def calc_dist(rank):
    ...
    means = list(gb.result.dist.mean)
...
def main():
    ...
    for _ in range(TEST_NR):
        rank = [random.choose([1, 2, 3]) for x in range(len(OUR_RANK))]
        for ifca, mean in zip(calc_dist(rank)):
            ifca_raw[ifca].append(mean)
            ifca_closer_agreement.append(1 if actual_mean_distances[actual_ifcas.index(ifca)] < mean else 0)
    ...
    output = [['ifca', 'mean_distance', 'n', 'closer_n', 'p']]

    for k, v in ifca_raw.items():
        output.append([k, actual_mean_distances[actual_ifcas.index(k)], TEST_NR, v.count(1), v.count(1)/TEST_NR])

    #add the overall record
    f = all_closer_agreement.count(1)
    output.append(['all', MEAN_DISTANCE, TEST_NR, f, f/TEST_NR])
    iolib.write_csv(CSV, output, inner_as_rows=False)
