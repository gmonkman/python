...
with wordcount from gazetteer_shore.dbo.gazetteer_afloat
...
import gazetteerdb
from gazetteerdb.model import t_v_gazetteer_afloat_word_count as T
...
files_delete2(settings.PathsUGCGazAfloat.LOG_MAKE_GAZ_WORDCOUNTS)
Log = Logger(name='train', logToStdout=False, logToFile=True, logFileMaxSize=1)
Log.set_log_file(settings.PathsUGCGazAfloat.LOG_MAKE_GAZ_WORDCOUNTS)
print('\nLogging to %s\n' % settings.PathsUGCGazAfloat.LOG_MAKE_GAZ_WORDCOUNTS)
...
def main():
    ...
    row_cnt = gazetteerdb.SESSION.query(T).order_by(T.columns.gazetteer_afloatid).slice(OFFSET, max_row).count()
    ...
    while True:
        ...
        rows = gazetteerdb.SESSION.query(T).order_by(T.columns.gazetteer_afloatid).slice(start, stop).all()
...
if __name__ == "__main__":
    main()
    iolib.pickle(D, settings.PathsUGCGazAfloat.GAZ_WORDS_BY_WORD_COUNT)
    print('\nDone. Saved to %s' % settings.PathsUGCGazAfloat.GAZ_WORDS_BY_WORD_COUNT) 
