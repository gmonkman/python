...
class NamedEntityBase():
    ...
    @classmethod
    def get(cls, add_similiar=False, force_conjugate=False, typos=('nouns', 'verbs', 'phrases', 'others'), force_plural_singular=False):
...
class AFLOAT_CB(NamedEntityBase):
    '''doc'''
...
class AFLOAT_PRIVATE(NamedEntityBase):
    '''doc'''
...
class AFLOAT_KAYAK(NamedEntityBase):
    '''kayak'''
    ADJECTIVES = []
...
class METROLOGICAL(NamedEntityBase):
    '''doc'''
...
class GEAR_ANGLING(NamedEntityBase):
    '''doc'''
    ADJECTIVES = []
...
class GEAR_NONE_ANGLING(NamedEntityBase):
    '''doc'''
    PHRASES = ['spear gun', 'long lines', 'long line', 'purse net', 'seine']


class SPECIES(NamedEntityBase):
    '''spp'''
