...
class Rom():
    def __init__(self, full_path_filename):
        ...
        self.base_filename, self.stripped_filename, self.tokens, self.base_filename = self.describe_rom(full_path_filename)
...
if __name__ == "__main__":
    ...
    all_roms = ALL_ROMS(os.path.normpath(args.rom_dir), args.delete)
    # get the list of games:
    game_list = all_roms.get_roms()
    # create a rom object for each file:
    for full_path_filename in game_list:
        try:
            R = Rom(os.path.normpath(full_path_filename))
            all_roms.add_rom(R)
        except:
            pass
