...
def main():
    ...
    parser.add_argument('-r','--recurse', help='Recurse folders', action='store_true')
    parser.add_argument('-o','--outfile', help='Output batch file')
    parser.add_argument('f', '--futurize_exe', help='Path to futurize executable', default=r'C:\Python352\Scripts\futurize.exe')
    parser.add_argument('e', '--execute_script', help='execute the script after creation', action='store_true')
    parser.add_argument('dirs', help='Folders to check', nargs='+')
    args = parser.parse_args()

    flds = [path.normpath(path.join(f,'*.py')) for f in file_list_glob_generator(args.dirs,recurse=True)]
