    ...
    * im_window is the sliding window image
    ...
    for y in range(0, image.shape[0], step_sz[1]):
        for x in range(0, image.shape[1], step_sz[0]):
            img = image[y:y + win_sz[1], x:x + win_sz[0]]
            if discard_partial:
...
@decgetimg
def pyrwin(image, scale=1.5, min_pyr_size=(100,100), discard_partial=True, win_func=slide_win_abs, **win_func_args):
    ...
    '''
    for resized in pyramid(image, scale=scale, min_pyr_size=min_pyr_size): # loop over the image pyramid
	    # loop over the sliding window for each layer of the pyramid
        for x, y, window in sliding_window(resized, **win_func_args):
            yield (x, y, window)
