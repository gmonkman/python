...
def load_base(fn):
    a = np.loadtxt(fn, np.float32, delimiter=',', converters={
...
class LetterStatModel(object):
    ...
    def unroll_samples(self, samples):
        ...
        new_samples = np.zeros(
            (sample_n * self.class_n, var_n + 1), np.float32)
...
class Boost(LetterStatModel):
    ...
    def train(self, samples, responses):
        ...
        var_types = np.array([cv2.ml.VAR_NUMERICAL] * var_n +
                             [cv2.ml.VAR_CATEGORICAL, cv2.ml.VAR_CATEGORICAL], np.uint8)

        self.model.setWeakCount(15)
        self.model.setMaxDepth(10)
        self.model.train(cv2.ml.TrainData_create(
            new_samples, cv2.ml.ROW_SAMPLE, new_responses.astype(int), varType=var_types))
...
class MLP(LetterStatModel):
    ...
    def train(self, samples, responses):
        sample_n, var_n = samples.shape
        new_responses = self.unroll_responses(
            responses).reshape(-1, self.class_n)
...
if __name__ == '__main__':
    ...
    args, dummy = getopt.getopt(
        sys.argv[1:], '', ['model=', 'data=', 'load=', 'save='])
    args = dict(args)
    ...
    print('testing...')
    train_rate = np.mean(model.predict(
        samples[:train_n]) == responses[:train_n].astype(int))
    test_rate = np.mean(model.predict(
        samples[train_n:]) == responses[train_n:].astype(int))

    print('train rate: %f  test rate: %f' %
