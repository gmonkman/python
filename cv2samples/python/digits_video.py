...
def main():
    ...
    while True:
        ...
        _, contours, heirs = cv2.findContours(
            bin.copy(), cv2.RETR_CCOMP, cv2.CHAIN_APPROX_SIMPLE)
        try:
            heirs = heirs[0]
        except:
            heirs = []
        ...
        for cnt, heir in zip(contours, heirs):
            ...
            bin_norm = cv2.warpAffine(
                bin_roi, A, (SZ, SZ), flags=cv2.WARP_INVERSE_MAP | cv2.INTER_LINEAR)
            ...
            cv2.putText(frame, '%d' % digit, (x, y),
                        cv2.FONT_HERSHEY_PLAIN, 1.0, (200, 0, 0), thickness=1)
